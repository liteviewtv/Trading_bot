...
def inspect_positions(symbol, max_open_positions=5, *, cooldown_minutes=30, current_positions=None, current_orders=None, historical_orders=None):
    if max_open_positions < 0: raise ValueError("max_open_positions must be non-negative")
    if cooldown_minutes < 0: raise ValueError("cooldown_minutes must be non-negative")
    ...
    if cooldown_minutes > 0:
        ...
        for order in history:
            if str(order.get("symbol", "")) != symbol: continue
            if str(order.get("side", "")).lower() != "buy": continue
            timestamp = order.get("submitted_at") or order.get("created_at") or order.get("updated_at")
            if not timestamp: continue
            try:
                ts = datetime.fromisoformat(str(timestamp).replace("Z", "+00:00"))
                if ts >= cutoff:
                    return PositionState(len(current), symbols, False, f"Recent BUY order exists for symbol (cooldown {cooldown_minutes}m)")
            except ValueError:
                continue
